...
export const useImportExcelStore = defineStore('importExcelStore', {
    ...
    actions: {
        async previewExcelUpload(file, router) {
        ...
        async onUploadValidateData(body) {
            ...
            try {
                this.eventSource = new AuthenticationEventSource(`${apiUrl}/v1/auth/users/me/shipments/management/imports/pre-shipment/progress/${body.import_id}`, `${this.userStore.token}`);
            ...
            try {
                this.isLoading = true;
                const res = await api.post("v1/auth/users/me/shipments/management/imports/pre-shipment", body, {
                    headers: {
        ...
        async onSavePayloadData(body, router) {
            ...
            try {
                this.isLoading = true;
                const res = await api.post("v1/auth/users/me/shipments/management/imports/pre-shipment/payload", body);
                console.log(res);
                this.isDirty = false;
                notification.success({
                    message: "ສຳເລັດ!",
        ...
        validateRow(row) {
            ...
            for (const index of requiredIndices) {
                ...
                if (!row.data[index] || !row.colErrors[index])
                    continue;
        ...
        watchRowsData() {
            ...
            this.groupedRows.forEach(row => {
                if (!this.rowWatchers.has(row.rowIndex)) {
                    const unwatch = watch(() => [...row.data], () => this.validateRow(row), { deep: true });
