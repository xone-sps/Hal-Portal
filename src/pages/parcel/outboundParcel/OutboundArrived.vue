  ...
  <div class="px-3">
    ...
    <a-card class="custom-table-card">
      <div class="flex justify-between items-center !mb-4">
        <div>
          <p class="text-lg font-semibold">ລາຍການພັດສະດຸຮອດສາຂາປາຍທາງ <span class="text-red-500">{{ outboundStore.outboundList?.lenght }}</span></p>
        </div>
...
const handleSearch = () => {
  if(startDate.value != null && endDate.value != null){
    outboundStore.fetchOutboundData({status: 'arrived_status',query:searchQuery.value,startDate:startDate.value.format('YYYY-MM-DD'),endDate:endDate.value.format('YYYY-MM-DD')},);
  }
};
...
onMounted(async () => {
  await outboundStore.fetchOutboundData({status: 'arrived_status',startDate:startDate.value.format('YYYY-MM-DD'),endDate:endDate.value.format('YYYY-MM-DD')});
});
