  ...
  <div class="px-3">
    ...
    <a-card class="!mb-1">
      <div class="flex justify-between items-center">
        ...
        <a-button type="primary" class="export-button !text-white"  :loading="exportStore.isExportLoading"
                  @click="handleExport">
          Export Excel
        </a-button>
      </div>
...
const handleSearch = async () => {
  ...
  await inboundStore.fetchInboundData({status: 'success',query:searchQuery.value,startDate:startDate.value,endDate:endDate.value});
  }
};
...
onMounted(async () => {
  await inboundStore.fetchInboundData({status: 'success',startDate:startDate.value,endDate:endDate.value});
});
</script>
