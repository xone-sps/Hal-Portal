...
const handlePaginate = (cursor: string) => {
  inboundStore.fetchInboundData({status: 'returned_success',cursor:cursor,startDate:startDate.value,endDate:endDate.value});
};
const handleSearch = async () => {
  if(startDate.value !== '' && endDate.value !== ''){
    await inboundStore.fetchInboundData({status: 'returned_success',query:searchQuery.value,startDate:startDate.value,endDate:endDate.value});
  }
};
...
onMounted(async () => {
  await inboundStore.fetchInboundData({status: 'returned_success',startDate:startDate.value,endDate:endDate.value});
});
</script>
