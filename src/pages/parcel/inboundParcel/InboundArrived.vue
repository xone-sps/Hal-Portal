<template>
  <div class="px-3">
    ...
    <a-card class="custom-table-card">
      ...
      <a-table :columns="columns" :data-source="inboundStore.inboundList" :pagination="false" :locale="{ emptyText: $emptyText() }">
...
const handlePaginate = (cursor: string) => {
  inboundStore.fetchInboundData({status:'arrived_status',cursor:cursor,startDate:startDate.value,endDate:endDate.value});
};
const handleSearch = () => {
  if(startDate.value != '' && endDate.value != ''){
    inboundStore.fetchInboundData({status:'arrived_status',query:searchQuery.value, startDate:startDate.value,endDate:endDate.value});
  }
};
...
onMounted(async () => {
  await inboundStore.fetchInboundData({status: 'arrived_status',startDate:startDate.value,endDate:endDate.value});
});
